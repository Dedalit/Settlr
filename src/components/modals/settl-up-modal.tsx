    ...
    <Modal open={open} onClose={onClose} title="Settl Up">
      {empty ? (
        <div className="py-8 text-center">
          <p className="text-sm text-purple-200/60">You're all settled up.</p>
          <Button
            onClick={onClose}
            className="mt-6 h-11 w-full rounded-full bg-purple-500/80 text-white hover:bg-purple-400"
          >
            Done
          </Button>
        </div>
        ...
        <div className="space-y-5">
          {showPicker && (
            <div>
              <p className="mb-3 text-sm font-medium text-purple-200/70">
              ...
              <div className="space-y-2">
                {targets.map((t, i) => (
                  ...
                  >
                    ...
                    <span className="flex-1 text-left text-sm font-semibold text-white">{t.name}</span>
                    {selected === i ? (
                      <CheckCircle2 className="size-4 text-purple-400" />
                    ) : (
                      <Circle className="size-4 text-purple-200/40" />
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-center">
            ...
            {!showPicker && <p className="mt-2 text-sm font-semibold text-white">{active?.name}</p>}
            <p className="mt-1 text-xs text-purple-200/50">You'll settle</p>
            <p className="mt-1 text-3xl font-black text-white">€{active?.amount.toFixed(2)}</p>
          </div>

          <Button className="h-12 w-full rounded-full border-orange-400/50 bg-orange-500 text-base font-semibold text-white shadow-lg shadow-orange-500/30 hover:bg-orange-400">
